...
class Album(models.Model):
    ...
    def thumbnailed_photos(self):
        return self.photo_set.exclude(
            Q(file="") | Q(file__isnull=True) | Q(thumbnailed_at__isnull=True)
        ).order_by("-taken_at")
...
class Photo(models.Model):
    ...
    # File uploaded to S3
    file = models.ImageField(upload_to=get_s3_key, null=True, blank=True)
    # Local file on disk
    local_file = models.ImageField(
        upload_to=get_local_filename, storage=file_storage, null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.uploaded_at:
            async_task(upload_photo_to_s3, self.pk)
